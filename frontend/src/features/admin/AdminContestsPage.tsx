    ...
    const handleSave = async () => {
        ...
        try {
            const payload = {
                ...
                subClubName: currentContest.subClubName,
                        ...
                        <Card className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-lg overflow-hidden">
                            ...
                            <CardContent className="space-y-5 bg-white dark:bg-gray-800 p-6">
                                ...
                                <Select
                                    ...
                                    onChange={e => setCurrentContest({ ...currentContest, clubId: e.target.value, subClubName: '' })}
                                    options={clubs.map(c => ({ value: c.id, label: c.name }))}
                                />
                                {(() => {
                                    const selectedClub = clubs.find(c => c.id === currentContest.clubId);
                                    const subClubs = selectedClub?.subClubGroup?.subClubs || [];
                                    if (subClubs.length > 0) {
                                        return (
                                            <Select
                                                label={`${selectedClub?.subClubGroup?.name || 'Sub-Club'}`}
                                                value={currentContest.subClubName || ''}
                                                onChange={e => setCurrentContest({ ...currentContest, subClubName: e.target.value })}
                                                options={[
